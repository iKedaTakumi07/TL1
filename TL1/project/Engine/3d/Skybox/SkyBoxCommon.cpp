...
{
    static std::unique_ptr<SkyBoxCommon> instance = std::make_unique<SkyBoxCommon>(ConstructorKey());
...
void SkyBoxCommon::PrepareObjectDraw()
{
...
void SkyBoxCommon::RootSignatureInitialize(DirectXCommon* dxcommon)
{
    // RootSignature作成
...
void SkyBoxCommon::graphicsPipelineInitialize(DirectXCommon* dxcommon)
{
    // ルートシグネチャ作成
    ...
    Microsoft::WRL::ComPtr<IDxcBlob> pixeShaderBlob = dxcommon->CompileShader(L"resources/shaders/Skybox.PS.hlsl", L"ps_6_0");
